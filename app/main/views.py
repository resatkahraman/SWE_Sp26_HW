...
def calculate():
    ...
    if form.validate_on_submit():
       ...
       if tle_dict:
          ...
          if error == 0:
             # tle_date_time kısmına tle_dict['date'] verisini ekledik
             result = {"tle_date_time":f"TLE DateTime: {tle_dict['date']}", "l1":f"Position (X, Y, Z) in km: {r}", "l2":f"Velocity (dx, dy, dz) in km/s: {v}", "msg":"Satellite/space object position and velocity calculated"}
          else:
